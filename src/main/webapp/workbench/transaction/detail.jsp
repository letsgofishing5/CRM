...
<%@ page import="com.cth.crm.settings.domain.DicValue" %>
<%@ page import="com.cth.crm.workbench.domain.Tran" %>
...
<%
	String basePath = request.getScheme() +
			...
			"/";
//	获取数据字典的值
	List<DicValue> dvList = (List) application.getAttribute("stage");
//	准备阶段和可能性的关系
	Map<String,String> map = (Map<String, String>) application.getAttribute("stageMap");
//	准备map中的key
...
//		如果可能性为0，说明找到了前面正常阶段的和后面丢失阶段的临界点
...
<head>
	...
	<script type="text/javascript">
		...
		function showHistoryList(){
			$.ajax({
				url:"workbench/transaction/historyList.do",
				type:"get",
				data:{
					"tranId":"${tran.id}",
				},
		...
		function changIcon(stage,index1) {
			...
			var index = index1;
			var point = "<%=point%>";
			//如果当前阶段可能性为0,前7个是黑圈，后面一个是黑叉一个是红叉
			if("0"==currentPossibility ||"0sys"==currentPossibility)
			{
				//遍历前7个
...
//		判断当前阶段是否为0
		if ("0".equals(currentPossibility)||"0sys".equals(currentPossibility))
		{
			for(int i=0;i<dvList.size();i++)
			{
				//取得每一个遍历出来的阶段，根据每个遍历出来的阶段取其可能性
				DicValue dv = dvList.get(i);
				String listStage = dv.getValue();
				String listPossibility = map.get(listStage);
				//如果遍历出来的阶段可能性为0，说明是后两个，
				if("0".equals(listPossibility)||"0sys".equals(listPossibility))
				{//如果是当前阶段
	...
	<%
		...
		//如果当前阶段不为0，前7个有可能是绿圈，黑圈，后两个一定是黑叉
	}else{
		//准备当前阶段的下标
		int index=0;
		for (int i=0;i<dvList.size();i++)
	...
	-------------
	<%
		//如果遍历出来的阶段可能性不为0，说明前7个阶段，绿色标记，黑圈
	}else{
		//如果是当前阶段，
		if (i==index)
		{
...
<div style="position: relative; top: 0px;">
	...
	<div style="position: relative; left: 40px; height: 30px; top: 10px;">
		...
		<div style="width: 300px;position: relative; left: 200px; top: -20px;"><b>${tran.customerId}-${tran.contactsId}&nbsp;</b></div>
	...
	<div style="position: relative; left: 40px; height: 30px; top: 60px;">
		...
		<div style="width: 500px;position: relative; left: 200px; top: -20px;"><b>${user.name}&nbsp;&nbsp;</b><small style="font-size: 10px; color: gray;">${tran.createTime}&nbsp;</small></div>
		<div style="height: 1px; width: 550px; background: #D5D5D5; position: relative; top: -20px;"></div>
	</div>
